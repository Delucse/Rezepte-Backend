...
favorite.post('/:recipeId', authorization, async (req, res) => {
  try {
    const user = await User.findOneAndUpdate({_id: req.user.id, favorites: {$not: {$in: [req.params.recipeId]}}}, {$push: {favorites: req.params.recipeId } });
    if(user){
      res.status(201).send({ msg: 'created recipe favorite successfully'});
    } else {
...
favorite.delete('/:recipeId', authorization, async (req, res) => {
  try {
    const user = await User.findByIdAndUpdate(req.user.id, {$pull: {favorites: req.params.recipeId } });
